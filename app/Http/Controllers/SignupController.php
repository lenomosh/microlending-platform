    ...
    public function fetch (Request $request){
        ...
        $validator= Validator::make($request->all(),
            [
                ...
                'dob'=>['date','required', new AgeOfAdult],
                'idno'=>['required',
                    'regex:/^\d+$/',
                    'min:7',
                    'max:8',
            ...
            $messages = array(
                ...
                'dob.date'=>'Kindly put a correct date. Dates should be separated with a forward slash. eg 10/20/2000',
            )
            );
        if ($validator->fails()) {
...
//            header("refresh:5; url=regcom");
    ...
    public function initiate(Request $request){
        ...
        $PartyB = "739283";
        $PhoneNumber = $snumber;
        $CallBackURL = "http://goafrica-kenyagrantmakers.com/mpesacall";
...
//            $execute = $conn->query($query);
                $request->session()->flush();
            echo '<script type="text/javascript">alert("'.$message.'");</script>';
            header("refresh:2; url=http://goafrica-kenyagrantmakers.com");
        } else {
$message = "Payment not successful please try again";
echo '<script type="text/javascript">alert("'.$message.'");</script>';
header("refresh:1; url=http://goafrica-kenyagrantmakers.com/regcom");
}
